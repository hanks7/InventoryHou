...
public class PermissionReq {
    ...
    public static boolean judgePermisson(final Context context, String permissionType) {
        String str = null;
        if (isHasPerssion(context, permissionType)) {
            return true; //有这个权限
        } else {
            ...
            if (permissionType.contains("READ_EXTERNAL_STORAGE")) {
                str = "允许程序读取外部存储";
            }
            ...
            showDialog(context, context.getString(R.string.permission_note, str));
            return false; //没有这个权限
        }
    }
     ...
     * @return
     */
    public static boolean isHasPerssion(final Context context, String permissionType) {
