...
public class UTools {
     ...
     *
     * @param activity
     * @param isShowToast  是否弹出吐司
     * @param isShowDialog 是否显示弹出dialog
     */
    public static void netUpdate(final BaseActivity activity, final boolean isShowToast, boolean isShowDialog) {
        HttpAdapter.getService().autoUpdate().enqueue(new OnResponseListener<UpdateBean>(isShowDialog ? activity : null) {
            @Override
            public void onSuccess(UpdateBean bean) {
                UpdateManager updateManager = new UpdateManager(activity, bean);
                updateManager.showDownloadDialog(isShowToast);
            }
